...
typedef enum {
	...
	PARSER_CHAR_NUMERIC_NEGATIVE_SIGN = 1 << 8,
	PARSER_CHAR_WHITESPACE = 1 << 9,
	PARSER_CHAR_STRING_DELIMITER = 1 << 10,
	PARSER_CHAR_STRING_ESCAPE = 1 << 11
} EParserChar_Flags;
...
void g_Parser_setupCharLookupTable(void) {
	...
	tbl['-'] |= PARSER_CHAR_NUMERIC_NEGATIVE_SIGN;
