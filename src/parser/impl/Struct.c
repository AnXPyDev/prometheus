MemberList *Parser_parseStructMembers(TokenStream *ts, ParserContext *ctx, ParserResult *out) {
	...
	while (true) {
		...
		result.flags =  PARSENODE_FLAG_NO_CONSUME_EXPLICIT_END |  PARSENODE_FLAG_ALLOW_INTRIN | PARSENODE_FLAG_DEFER_DECLARATION;
		...
		if (0) err_invalid_type: {
			Parser_throws(ctx, &here->src, PARSER_RESULT_PANIC, "Non parsetime expression used in struct type", out);
			return NULL;
		}
