bool ParseTree_sub_number(ParseTree *this, TokenStream *ts, ParseTreeState **statep, void *payload) {
	Token *token = *(Token**)payload;

	ParserResult result = ParserResult_NULL;
	Parser_parseNumber(token, this->ctx, &result);
...
void ParseTree_branch_number(ParseTree *this, Token *token, ParseTreeState *state) {
	switch (state->type) {
		default:;
			return;
...

	{
		...
		option->subf= &ParseTree_sub_number;
